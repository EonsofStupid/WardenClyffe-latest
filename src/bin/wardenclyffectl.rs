...
async fn main() {
    ...
    let result = match &cli.command {
        ...
        Commands::TuneMariadb { output, no_restart, dry_run } => {
            tune_mariadb(output.clone(), *no_restart, *dry_run)
        }
...
fn tune_mariadb(
    ...
    // Detect system RAM
    let total_ram_kb = get_total_ram_kb()?;
    let total_ram_gb = total_ram_kb as f64 / 1024.0 / 1024.0;
    println!("\x1b[1;32m✓\x1b[0m Detected RAM: {:.1} GB", total_ram_gb);

    // Detect CPU cores
    let cpu_cores = std::thread::available_parallelism()
        .map(|p| p.get())
        .unwrap_or(4);
    println!("\x1b[1;32m✓\x1b[0m Detected CPU cores: {}", cpu_cores);

    // Calculate optimal buffer pool (70% of RAM, but leave at least 1GB for OS)
    let buffer_pool_gb = ((total_ram_gb * 0.7).floor() as u64).max(1);
    let buffer_pool_instances = (buffer_pool_gb as usize).min(8).max(1);
    
    println!();
    println!("Recommended settings:");
    println!("  innodb_buffer_pool_size = {}G", buffer_pool_gb);
    println!("  innodb_buffer_pool_instances = {}", buffer_pool_instances);
...
[mariadb]
# Buffer pool - {:.1}GB RAM detected, using {}GB (70%)
...
# Connections & threading ({} CPU cores detected)
max_connections = 500
thread_pool_size = {}
...
"#, 
        ...
        total_ram_gb, buffer_pool_gb,
        buffer_pool_gb, buffer_pool_instances,
        cpu_cores, cpu_cores
    );
    ...
    if dry_run {
        println!();
        println!("\x1b[1;33m[DRY RUN]\x1b[0m Would write to: {}", output_path);
        println!();
        ...
        println!("─────────────────────────────────────");
        if !no_restart {
            println!("\x1b[1;33m[DRY RUN]\x1b[0m Would restart MariaDB");
        }
        return Ok(());
    }
    ...
    // Write config file
    println!();
    println!("Writing config to: {}", output_path);
    ...
    // Try to write directly first, then suggest sudo
    match std::fs::write(&output_path, &config_content) {
        Ok(_) => {
            println!("\x1b[1;32m✓\x1b[0m Config file written successfully");
        }
        Err(e) => {
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    println!("\x1b[1;32m✓\x1b[0m MariaDB restarted successfully");
                } else {
                    let stderr = String::from_utf8_lossy(&result.stderr);
                    if stderr.contains("Permission denied") || stderr.contains("Access denied") {
                        println!("\x1b[1;31m✗\x1b[0m Permission denied. Run with sudo to restart MariaDB:");
                        println!("  sudo systemctl restart mariadb");
                    } else {
                        println!("\x1b[1;31m✗\x1b[0m Failed to restart MariaDB: {}", stderr);
                    }
                }
            }
            Err(e) => {
                println!("\x1b[1;31m✗\x1b[0m Failed to run systemctl: {}", e);
                println!("  Manually restart MariaDB: sudo systemctl restart mariadb");
            }
        }
    } else {
        println!();
        println!("\x1b[1;33m!\x1b[0m Remember to restart MariaDB for changes to take effect:");
        println!("  sudo systemctl restart mariadb");
    }

    println!();
    println!("\x1b[1;32m✓ MariaDB tuning complete!\x1b[0m");
    println!();
...
fn get_total_ram_kb() -> Result<u64, Box<dyn std::error::Error>> {
