#!/bin/bash
#
# WardenClyffeNet Quick Install Script
# Installs WardenClyffeNet on Ubuntu/Debian (apt), Fedora/RHEL (dnf/yum),
# Arch/Manjaro/CachyOS (pacman), openSUSE (zypper), or Alpine (apk)
#
# Usage: curl -sSL https://raw.githubusercontent.com/wardenclyffesoftwaresystemsltd/WardenClyffeNet/main/setup.sh | sudo bash
#

set -e

echo ""
echo "  🐺 WardenClyffeNet Installer"
echo "  ─────────────────────────────────────"
...
if [ -n "$BINARY_ARCH" ]; then
    ...
    PREBUILT_URL="https://github.com/wardenclyffesoftwaresystemsltd/WardenClyffeNet/releases/latest/download"
    if curl -fSL --connect-timeout 10 --max-time 120 -o /tmp/wardenclyffenet "$PREBUILT_URL/wardenclyffenet-${BINARY_ARCH}" 2>/dev/null; then
        chmod +x /tmp/wardenclyffenet
        curl -fSL --connect-timeout 10 --max-time 60 -o /tmp/wardenclyffenetctl "$PREBUILT_URL/wardenclyffenetctl-${BINARY_ARCH}" 2>/dev/null && chmod +x /tmp/wardenclyffenetctl || true
        PREBUILT_OK=true
        echo "✓ Prebuilt binaries downloaded"
    else
        echo "⚠ Prebuilt binaries not available — will build from source"
        rm -f /tmp/wardenclyffenet /tmp/wardenclyffenetctl
    fi
fi
...
if [ "$PREBUILT_OK" = "false" ]; then
    ...
    if [ -d "$INSTALL_DIR" ]; then
        ...
        git clone https://github.com/wardenclyffesoftwaresystemsltd/WardenClyffeScale.git "$INSTALL_DIR"
        cd "$INSTALL_DIR"
    fi

    echo "✓ Repository cloned to $INSTALL_DIR"

    # Build WardenClyffeNet (as the real user if possible, to use their cargo)
    echo ""
    echo "Building WardenClyffeNet (this may take a few minutes)..."
    cd "$INSTALL_DIR/wardenclyffenet"

    if [ "$REAL_USER" != "root" ] && [ -f "$REAL_HOME/.cargo/bin/cargo" ]; then
        # Build as the real user so cargo uses their toolchain
        chown -R "$REAL_USER:$REAL_USER" "$INSTALL_DIR"
        su - "$REAL_USER" -c "cd $INSTALL_DIR/wardenclyffenet && $REAL_HOME/.cargo/bin/cargo build --release"
    else
        cargo build --release
    fi
...
if systemctl is-active --quiet wardenclyffenet 2>/dev/null; then
    echo ""
    echo "Stopping WardenClyffeNet service for upgrade..."
    systemctl stop wardenclyffenet
    sleep 2
...
# Install binary
echo ""
if [ -f "/usr/local/bin/wardenclyffenet" ]; then
    echo "Upgrading WardenClyffeNet..."
    rm -f /usr/local/bin/wardenclyffenet
else
    echo "Installing WardenClyffeNet..."
fi

if [ "$PREBUILT_OK" = "true" ]; then
    mv /tmp/wardenclyffenet /usr/local/bin/wardenclyffenet
    if [ -f /tmp/wardenclyffenetctl ]; then
        mv /tmp/wardenclyffenetctl /usr/local/bin/wardenclyffenetctl
        echo "✓ wardenclyffenetctl installed to /usr/local/bin/wardenclyffenetctl"
    fi
else
    cp "$INSTALL_DIR/wardenclyffenet/target/release/wardenclyffenet" /usr/local/bin/wardenclyffenet
    chmod +x /usr/local/bin/wardenclyffenet
    if [ -f "$INSTALL_DIR/wardenclyffenet/target/release/wardenclyffenetctl" ]; then
        cp "$INSTALL_DIR/wardenclyffenet/target/release/wardenclyffenetctl" /usr/local/bin/wardenclyffenetctl
        chmod +x /usr/local/bin/wardenclyffenetctl
        echo "✓ wardenclyffenetctl installed to /usr/local/bin/wardenclyffenetctl"
    fi
fi
echo "✓ wardenclyffenet installed to /usr/local/bin/wardenclyffenet"

# Create directories
echo ""
echo "Creating directories..."
mkdir -p /etc/wardenclyffenet
mkdir -p /var/run/wardenclyffenet
echo "✓ Directories created"

# Create config if not exists - with interactive prompts
# Use /dev/tty to read from terminal even when script is piped
if [ ! -f "/etc/wardenclyffenet/config.toml" ]; then
    echo ""
    echo "  $(printf '%0.s─' {1..50})"
    echo "  WardenClyffeNet Configuration"
    echo "  $(printf '%0.s─' {1..50})"
    echo ""
    ...
    # Prompt for WardenClyffeNet IP address
    echo -n "WardenClyffeNet IP address for this node [10.10.10.1]: "
    ...
    echo ""
    echo "Gateway mode enables NAT so other WardenClyffeNet nodes can access"
    ...
    KEY_FILE="/etc/wardenclyffenet/private.key"
    if [ ! -f "$KEY_FILE" ]; then
        /usr/local/bin/wardenclyffenet genkey --output "$KEY_FILE" 2>/dev/null || {
    ...
    # Get public key
    PUBLIC_KEY=$(/usr/local/bin/wardenclyffenet pubkey --config /etc/wardenclyffenet/config.toml 2>/dev/null || \
                 /usr/local/bin/wardenclyffenet pubkey 2>/dev/null || echo "unknown")

    # Write config
    echo ""
    echo "Creating configuration..."
    cat <<EOF > /etc/wardenclyffenet/config.toml
# WardenClyffeNet Configuration
# Generated by setup.sh

[network]
interface = "wardenclyffenet0"
...
# name = "server2"
EOF
    echo "✓ Config created at /etc/wardenclyffenet/config.toml"
    echo ""
    echo "Configuration Summary:"
    echo "  WardenClyffeNet IP: $NODE_ADDRESS/$SUBNET"
    echo "  Listen:     $LISTEN_PORT/udp"
    echo "  Gateway:    $IS_GATEWAY"
    echo "  Discovery:  $DISC_ENABLED"
else
    echo ""
    echo "✓ Config already exists at /etc/wardenclyffenet/config.toml"
    echo "  (Upgrade mode - skipping configuration prompts)"
fi

# Create systemd service if not exists
if [ ! -f "/etc/systemd/system/wardenclyffenet.service" ]; then
    echo ""
    echo "  $(printf '%0.s─' {1..50})"
    ...
    cat > /etc/systemd/system/wardenclyffenet.service <<EOF
[Unit]
Description=WardenClyffeNet - Secure Private Mesh Networking
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/usr/local/bin/wardenclyffenet --config /etc/wardenclyffenet/config.toml
Restart=on-failure
RestartSec=5
LimitNOFILE=65535
...
# Status directory
RuntimeDirectory=wardenclyffenet
...
EOF
    ...
    echo ""
    echo -n "Start WardenClyffeNet now? [Y/n]: "
    read start_now < /dev/tty
    if [ "$start_now" != "n" ] && [ "$start_now" != "N" ]; then
        systemctl enable wardenclyffenet
        systemctl start wardenclyffenet
        sleep 2
        if systemctl is-active --quiet wardenclyffenet; then
            echo "✓ WardenClyffeNet is running!"
        else
            echo "⚠ WardenClyffeNet may have failed to start. Check: journalctl -u wardenclyffenet -n 20"
        fi
    else
        systemctl enable wardenclyffenet
        echo "✓ WardenClyffeNet enabled (will start on boot)"
    fi
else
    echo ""
...
if [ "$RESTART_SERVICE" = "true" ]; then
    echo ""
    echo "Restarting WardenClyffeNet service..."
    systemctl start wardenclyffenet
    sleep 2
    if systemctl is-active --quiet wardenclyffenet; then
...
elif systemctl is-enabled --quiet wardenclyffenet 2>/dev/null; then
    echo ""
    echo "Starting WardenClyffeNet service..."
    systemctl start wardenclyffenet
    sleep 2
    if systemctl is-active --quiet wardenclyffenet; then
...
echo "  ─────────────────────────────────────"
echo "  Status:   sudo systemctl status wardenclyffenet"
echo "  Logs:     sudo journalctl -u wardenclyffenet -f"
echo "  Config:   /etc/wardenclyffenet/config.toml"
echo ""
