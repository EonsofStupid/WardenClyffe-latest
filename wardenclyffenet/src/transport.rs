//! Network transport layer for WardenClyffeNet
//!
...
const DISCOVERY_PREFIX: &str = "WARDENCLYFFENET";

/// Build a handshake packet:
/// [1: type] [32: public_key] [4: wardenclyffenet_ip] [2: listen_port] [1: is_gateway] [N: hostname]
pub fn build_handshake(keypair: &KeyPair, wardenclyffenet_ip: Ipv4Addr, listen_port: u16, hostname: &str, is_gateway: bool) -> Vec<u8> {
    let mut pkt = Vec::with_capacity(40 + hostname.len());
    pkt.push(PKT_HANDSHAKE);
    pkt.extend_from_slice(keypair.public.as_bytes());
    pkt.extend_from_slice(&wardenclyffenet_ip.octets());
...
pub fn send_handshakes(
    ...
    wardenclyffenet_ip: Ipv4Addr,
    listen_port: u16,
    hostname: &str,
    is_gateway: bool,
) {
    let handshake = build_handshake(keypair, wardenclyffenet_ip, listen_port, hostname, is_gateway);
...
pub fn format_discovery(
    ...
    format!(
        ...
        wardenclyffenet_ip, listen_port, hostname,
...
pub fn parse_discovery(message: &str) -> Option<(String, x25519_dalek::PublicKey, Ipv4Addr, u16, String, bool)> {
    ...
    let wardenclyffenet_ip: Ipv4Addr = parts[4].parse().ok()?;
    let listen_port: u16 = parts[5].parse().ok()?;
    let hostname = parts[6].to_string();
    let is_gateway = parts[7] == "G";
    Some((node_id, public_key, wardenclyffenet_ip, listen_port, hostname, is_gateway))
}
...
pub struct PexEntry {
    ...
    /// WardenClyffeNet IP address
    pub wardenclyffenet_ip: String,
...
pub fn run_discovery_broadcaster(
    ...
    while running.load(std::sync::atomic::Ordering::Relaxed) {
        let msg = format_discovery(&node_id, &public_key, wardenclyffenet_ip, listen_port, &hostname, is_gateway);
        ...
        match socket.recv_from(&mut buf) {
            Ok((len, src)) => {
                if let Ok(msg) = std::str::from_utf8(&buf[..len]) {
                    if let Some((node_id, pub_key, wardenclyffenet_ip, listen_port, hostname, is_gateway)) = parse_discovery(msg) {
                        ...
                        peer_manager.update_from_discovery(&pub_key, endpoint, wardenclyffenet_ip, &hostname, is_gateway);
