//! Peer management for WardenClyffeNet
//!
...
pub struct Peer {
    ...
    pub peer_id: [u8; 4],
    /// Peer's IP on the WardenClyffeNet virtual network
    pub wardenclyffenet_ip: Ipv4Addr,
    ...
    /// This is the WardenClyffeNet IP of the peer that shared this entry with us
    ...
    /// on 2026-05-11 where wardenclyffenetctl said "6 (6 connected)" while
...
impl Peer {
    ...
    pub fn new(public_key: PublicKey, wardenclyffenet_ip: Ipv4Addr) -> Self {
...
pub struct PeerManager {
    /// Peers indexed by WardenClyffeNet IP
    peers_by_ip: Arc<RwLock<HashMap<Ipv4Addr, Peer>>>,
    /// Peer ID → WardenClyffeNet IP mapping for fast packet routing
    id_to_ip: Arc<RwLock<HashMap<[u8; 4], Ipv4Addr>>>,
    /// Endpoint → WardenClyffeNet IP mapping for incoming packet routing
    ...
    /// CIDR subnet routes: (network, prefix, gateway WardenClyffeNet IP). Sorted
    /// longest-prefix first so find_subnet_match can return on the
    /// first hit. Populated from /var/run/wardenclyffenet/subnet-routes.json,
    /// which WardenClyffe writes when its WardenClyffeRouter SubnetRoute config
    /// changes. Without this, packets read from the TUN whose dest IP
    /// matches a kernel route via wardenclyffenet0 (e.g. a remote LAN like
...
/// peer endpoints — a wardenclyffenet daemon receiving a PEX entry that
...
///
/// klasSponsor 2026-05-12 architectural point: "WardenClyffenet should not
...
pub fn is_routable_endpoint(addr: SocketAddr) -> bool {
    match addr.ip() {
        ...
        std::net::IpAddr::V6(_) => true, // IPv6 passthrough — wardenclyffenet is IPv4-focused today
    }
}
...
impl PeerManager {
    ...
    pub fn add_peer(&self, peer: Peer) {
        let ip = peer.wardenclyffenet_ip;
    ...
    /// Remove a peer by WardenClyffeNet IP (used for purging stale PEX entries)
    ...
    /// Get a mutable reference to a peer by WardenClyffeNet IP (via callback to avoid lock issues)
    ...
    pub fn update_from_discovery(&self, public_key: &PublicKey, endpoint: SocketAddr, wardenclyffenet_ip: Ipv4Addr, hostname: &str, is_gateway: bool) {
        let mut peers = self.peers_by_ip.write().unwrap();
        
        // First check: does a peer with this exact IP exist?
        if let Some(peer) = peers.get_mut(&wardenclyffenet_ip) {
            if peer.public_key == *public_key {
                ...
                peer.relay_via = None;
                self.endpoint_to_ip.write().unwrap().insert(endpoint, wardenclyffenet_ip);
                return;
            }
        }
        ...
        if let Some(old_ip) = existing_ip {
            if old_ip != wardenclyffenet_ip {
                // Peer changed their WardenClyffeNet IP — migrate to new IP
                ...
                peer.wardenclyffenet_ip = wardenclyffenet_ip;
                peer.endpoint = Some(endpoint);
                peer.hostname = hostname.to_string();
                peer.is_gateway = is_gateway;
                peer.relay_via = None;
                ...
                // Update all mappings
                self.endpoint_to_ip.write().unwrap().insert(endpoint, wardenclyffenet_ip);
                self.id_to_ip.write().unwrap().insert(peer_id, wardenclyffenet_ip);
                peers.insert(wardenclyffenet_ip, peer);
                return;
            }
        }
        ...
        if let Some(old_peer) = peers.remove(&wardenclyffenet_ip) {
            if let Some(old_ep) = old_peer.endpoint {
                self.endpoint_to_ip.write().unwrap().remove(&old_ep);
            }
            self.id_to_ip.write().unwrap().remove(&old_peer.peer_id);
        }
        let mut peer = Peer::new(*public_key, wardenclyffenet_ip);
        peer.endpoint = Some(endpoint);
        peer.hostname = hostname.to_string();
        peer.is_gateway = is_gateway;
        let peer_id = peer.peer_id;
    ...
    /// Returns the WardenClyffeNet IP of the first connected gateway peer
    ...
    /// Returns the WardenClyffeNet IP of the host that owns this container
    ...
    /// Load CIDR-based subnet routes from a JSON file (cidr → gateway WardenClyffeNet IP).
    /// Called on startup and on SIGHUP. WardenClyffe writes this file from its
    /// WardenClyffeRouter SubnetRoute config so userspace can do longest-prefix
    ...
    /// Returns the gateway WardenClyffeNet IP for the most specific configured
    ...
    /// architectural point from the wardenclyffe-side tombstone fix
    /// shipped in 22.14.11): "WardenClyffenet should not use internal ip for
    ...
    /// unreachable RFC1918 address as the peer's endpoint and wardenclyffenet
    ...
    pub fn get_pex_entries(&self, my_ip: Ipv4Addr) -> Vec<PexEntry> {
        let peers = self.peers_by_ip.read().unwrap();
        peers.values()
            .filter(|p| p.wardenclyffenet_ip != my_ip)
            .map(|p| {
                ...
                PexEntry {
                    ...
                    wardenclyffenet_ip: p.wardenclyffenet_ip.to_string(),
                    endpoint,
    ...
    pub fn add_from_pex(
        ...
        for entry in entries {
            // Skip ourselves
            let entry_ip: Ipv4Addr = match entry.wardenclyffenet_ip.parse() {
                Ok(ip) => ip,
                Err(_) => continue,
            };
            ...
            // link-local / multicast). Older wardenclyffenet versions (≤0.5.22)
    ...
    pub fn status(&self) -> Vec<crate::config::PeerStatus> {
        ...
        peers.values().map(|p| {
            crate::config::PeerStatus {
                hostname: p.hostname.clone(),
                address: p.wardenclyffenet_ip.to_string(),
...
mod subnet_match_tests {
    ...
    fn loaded_table_is_sorted_longest_first() {
        ...
        let pm = PeerManager::new();
        let tmp = std::env::temp_dir().join("wardenclyffenet-subnet-test.json");
        std::fs::write(
            &tmp,
    ...
    fn missing_file_clears_table() {
        ...
        let nonexistent = std::path::PathBuf::from("/tmp/wardenclyffenet-this-file-does-not-exist-zzz.json");
    ...
    fn malformed_json_keeps_table() {
        let pm = PeerManager::new();
        install(&pm, vec![("10.10.0.0", 16, "10.100.0.1")]);
        let tmp = std::env::temp_dir().join("wardenclyffenet-subnet-malformed.json");
