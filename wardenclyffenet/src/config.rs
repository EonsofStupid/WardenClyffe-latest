//! Configuration for WardenClyffeNet
...
pub struct NetworkConfig {
    ...
    /// Set to a specific IP to restrict which interface WardenClyffeNet listens on
...
pub struct PeerConfig {
    ...
    /// Peer's WardenClyffeNet IP address
...
fn default_interface() -> String { "wardenclyffenet0".into() }
fn default_subnet() -> u8 { 24 }
fn default_port() -> u16 { 9600 }
fn default_true() -> bool { true }
fn default_mtu() -> u16 { 1400 }
fn default_bind_address() -> String { "0.0.0.0".into() }
fn default_key_path() -> PathBuf { PathBuf::from("/etc/wardenclyffenet/private.key") }

/// Status information written by daemon, read by wardenclyffenetctl
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct PeerStatus {
    ...
    /// where `connected=true && data_flowing=false` is the "wardenclyffenet
    ...
    /// "never since wardenclyffenet started". Symmetric with `last_seen_secs`.
...
impl Config {
    ...
    pub fn load(path: &Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if migrated || removed > 0 {
            if migrated {
                eprintln!("[wardenclyffenet] Config migration: fixed 'ip' → 'allowed_ip' in {}", path.display());
            }
            if removed > 0 {
                eprintln!("[wardenclyffenet] Config cleanup: removed {} duplicate peer(s) from {}", removed, path.display());
            }
