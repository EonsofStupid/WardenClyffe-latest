//! TUN device management for WardenClyffeNet
//!
...
impl TunDevice {
    ...
    pub fn configure(&self, address: &str, subnet: u8, mtu: u16) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Disable IPv6 on the TUN — WardenClyffeNet is IPv4-only and the kernel pushes
        ...
        let nm_conf = "/etc/NetworkManager/conf.d/wardenclyffenet.conf";
        if !std::path::Path::new(nm_conf).exists()
            && std::path::Path::new("/etc/NetworkManager/conf.d").is_dir()
        {
            let _ = std::fs::write(nm_conf,
                "# WardenClyffeNet: prevent NetworkManager from managing the overlay interface.\n\
                 [keyfile]\n\
                 unmanaged-devices=interface-name:wardenclyffenet*\n");
        ...
        // traffic (port 41641) can get routed into wardenclyffenet0 via the subnet route,
        // creating a feedback loop where Tailscale traffic goes through WardenClyffeNet
