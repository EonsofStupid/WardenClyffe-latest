//! WardenClyffeNet daemon — secure private mesh networking
//!
...
use wardenclyffenet::config::{Config, NodeStatus};
use wardenclyffenet::crypto::KeyPair;
use wardenclyffenet::peer::{Peer, PeerManager};
use wardenclyffenet::tun::{self, TunDevice};
use wardenclyffenet::transport;

#[derive(Parser)]
#[command(name = "wardenclyffenet", version, about = "WardenClyffeNet — Secure private mesh networking")]
struct Cli {
    /// Path to config file
    #[arg(short, long, default_value = "/etc/wardenclyffenet/config.toml")]
    config: PathBuf,
...
enum Commands {
    ...
    Genkey {
        ...
        #[arg(short, long, default_value = "/etc/wardenclyffenet/private.key")]
        output: PathBuf,
    },
    ...
    Init {
        /// WardenClyffeNet IP address for this node
        #[arg(short, long, default_value = "10.0.10.1")]
        address: String,
    },
    /// Generate an invite token for a new peer to join your network
    Invite,
    /// Join a WardenClyffeNet network using an invite token
    Join {
        /// The invite token from 'wardenclyffenet invite'
        token: String,
    },
}
...
fn encrypt_and_send(
    send_buf: &mut [u8],
    plaintext: &[u8],
    peer: &mut wardenclyffenet::peer::Peer,
    my_peer_id: &[u8; 4],
    socket: &UdpSocket,
) -> bool {
...
fn main() {
    ...
    // Commands that need root access (for /etc/wardenclyffenet/)
    match &cli.command {
        Some(Commands::Invite) | Some(Commands::Join { .. }) | None => {
            if unsafe { libc::geteuid() } != 0 {
                eprintln!("✗ This command needs root access (to read /etc/wardenclyffenet/).");
                eprintln!("  Run with: sudo wardenclyffenet {}", std::env::args().skip(1).collect::<Vec<_>>().join(" "));
                std::process::exit(1);
            }
        }
        _ => {}
...
fn cmd_token(config_path: &PathBuf) {
    ...
    match KeyPair::load_or_generate(&config.security.private_key_file) {
        Ok(kp) => {
            ...
            println!("  wardenclyffenet join <token>");
        }
...
fn cmd_init(config_path: &PathBuf, address: &str) {
    let config = Config {
        network: wardenclyffenet::config::NetworkConfig {
    ...
    match config.save(config_path) {
        Ok(_) => {
            ...
            println!("WardenClyffeNet IP: {}", address);
            // Also generate key
            let kp = KeyPair::generate();
...
fn cmd_invite(config_path: &PathBuf) {
    ...
    println!();
    println!("  🐺 WardenClyffeNet Invite Token");
    println!("  ─────────────────────────────────────");
    ...
    println!();
    println!("  sudo wardenclyffenet --config /etc/wardenclyffenet/config.toml join {}", token);
    println!();
...
fn cmd_join(config_path: &PathBuf, token: &str) {
    use base64::Engine;
    use wardenclyffenet::config::PeerConfig;
    ...
    println!();
    println!("  🐺 WardenClyffeNet — Joined!");
    println!("  ─────────────────────────────────────");
    println!();
    println!("✓ Config saved to {:?}", config_path);
    println!("✓ Your WardenClyffeNet IP: {}/{}", config.network.address, subnet);
    println!("✓ Peer added: {} ({})", peer_ip, peer_endpoint);
    println!();
    println!("Now run this on the inviting node to complete the link:");
    println!();
    println!("  sudo wardenclyffenet --config /etc/wardenclyffenet/config.toml join {}", reverse_token);
    println!();
    println!("Then restart WardenClyffeNet on both nodes:");
    println!("  sudo systemctl restart wardenclyffenet");
}
...
fn run_daemon(config_path: &PathBuf) {
    let config = load_config(config_path);
    let wardenclyffenet_ip: Ipv4Addr = config.ip_addr().unwrap_or_else(|e| {
        error!("Invalid address '{}': {}", config.network.address, e);
        std::process::exit(1);
    });

    info!("WardenClyffeNet starting — {} on {}", wardenclyffenet_ip, config.network.interface);
    ...
    let tun = TunDevice::create(&config.network.interface).unwrap_or_else(|e| {
        ...
        error!("Are you running as root? (sudo wardenclyffenet)");
        std::process::exit(1);
    });
    ...
    for pc in &config.peers {
        match wardenclyffenet::crypto::parse_public_key(&pc.public_key) {
            Ok(pub_key) => {
    ...
    let routes_path = PathBuf::from("/var/run/wardenclyffenet/routes.json");
    peer_manager.load_routes(&routes_path);

    // Load CIDR-based subnet routes from WardenClyffe's WardenClyffeRouter config.
    // Without this, packets the kernel routes via wardenclyffenet0 toward a
    ...
    let subnet_routes_path = PathBuf::from("/var/run/wardenclyffenet/subnet-routes.json");
    ...
    if config.network.gateway {
        ...
        if let Err(e) = wardenclyffenet::gateway::enable_gateway(tun.name(), &subnet) {
            warn!("Gateway setup failed: {}", e);
        }
    }
    // Non-gateway nodes do NOT enable global ip_forward — WardenClyffeNet relay is
    ...
    if config.network.discovery {
        ...
        std::thread::spawn(move || {
            transport::run_discovery_broadcaster(wardenclyffenet_ip, pk, lp, h, gw, r);
        });
    ...
    // Spawn status writer thread
    {
        ...
        std::thread::spawn(move || {
            let status_path = PathBuf::from("/var/run/wardenclyffenet/status.json");
            std::fs::create_dir_all("/var/run/wardenclyffenet").ok();
    ...
    info!("WardenClyffeNet running — {} ({}) on {}", hostname, wardenclyffenet_ip, tun.name());
    ...
    while running.load(Ordering::Relaxed) {
            ...
            loop {
                ...
                if let Some(dest_ip) = tun::get_dest_ip(pkt) {
                    let subnet_broadcast = Ipv4Addr::new(
                        wardenclyffenet_ip.octets()[0], wardenclyffenet_ip.octets()[1],
                        wardenclyffenet_ip.octets()[2], 255,
                    );
                    ...
                    if dest_ip == subnet_broadcast || dest_ip == Ipv4Addr::BROADCAST {
                        ...
                        peer_manager.for_each_peer_mut(|ip, peer| {
                            if ip == wardenclyffenet_ip { return; }
                    ...
                    // CIDRs WardenClyffe advertised in subnet-routes.json. This
                    // is what lets a kernel route like
                    // `ip route add 10.10.0.0/16 via 10.100.10.30 dev wardenclyffenet0`
                ...
                match recv_buf[0] {
                    transport::PKT_HANDSHAKE => {
                        ...
                        if let Some((pub_key, peer_ip, _peer_port, is_gw, peer_hostname)) = transport::parse_handshake(data) {
                            ...
                            if !was_connected {
                                let reply = transport::build_handshake(&keypair, wardenclyffenet_ip, config.network.listen_port, &hostname, is_gateway);
                                ...
                                Ok(pt_len) => {
                                    ...
                                    if pt_len > 1 && plaintext[0] == transport::PKT_PEER_EXCHANGE {
                                        if let Some(entries) = transport::parse_peer_exchange(plaintext) {
                                            peer_manager.add_from_pex(&entries, peer_ip, wardenclyffenet_ip, &keypair);
                                        }
                                        continue;
                                    }
                                    ...
                                    if let Some(dest_ip) = tun::get_dest_ip(plaintext) {
                                        ...
                                        if dest_ip == wardenclyffenet_ip || dest_ip == subnet_bcast || dest_ip == Ipv4Addr::BROADCAST {
                                            ...
                                            if !forwarded {
                                                ...
                                                // back to wardenclyffenet0 since dest is in our subnet)
                                            }
                                        }
                                    }
        ...
        if last_handshake.elapsed() > Duration::from_secs(10) {
            transport::send_handshakes(&socket, &keypair, &peer_manager, wardenclyffenet_ip, config.network.listen_port, &hostname, is_gateway);
        ...
        if last_pex.elapsed() > Duration::from_secs(30) {
            transport::send_peer_exchange(&socket, &keypair, &peer_manager, wardenclyffenet_ip);
        ...
        //     from WardenClyffe without needing SIGHUP
                        ...
                        match wardenclyffenet::crypto::parse_public_key(&pc.public_key) {
                            Ok(pub_key) => {
                                ...
                                if existing_ips.contains(&ip) {
                                    ...
                                    //     WardenClyffe cluster-sync) intentionally removed
    ...
    // Cleanup
    info!("Shutting down WardenClyffeNet...");
    if config.network.gateway {
        wardenclyffenet::gateway::disable_gateway(tun.name(), &config.cidr());
    }
    let _ = std::fs::remove_file("/var/run/wardenclyffenet/status.json");
    info!("WardenClyffeNet stopped.");
}
