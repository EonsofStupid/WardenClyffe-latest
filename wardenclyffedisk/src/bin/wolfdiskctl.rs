//! WardenClyffeDiskCtl - Control utility for WardenClyffeDisk
//!
//! Usage:
//!   wardenclyffediskctl status          - Show node status from running service
//!   wardenclyffediskctl list servers    - List all discovered servers
//!   wardenclyffediskctl stats           - Live cluster statistics
...
/// WardenClyffeDisk Cluster Control Tool
#[derive(Parser)]
#[command(name = "wardenclyffediskctl")]
#[command(about = "Control and monitor WardenClyffeDisk clusters", long_about = None)]
struct Cli {
    /// Path to status file (written by running wardenclyffedisk service)
    #[arg(short, long, default_value = "/var/lib/wardenclyffedisk/cluster_status.json")]
    status_file: PathBuf,
...
fn read_status(path: &PathBuf) -> Result<ClusterStatus, Box<dyn std::error::Error>> {
    if !path.exists() {
        return Err(format!(
            "Status file not found: {}\n\nIs the wardenclyffedisk service running?\nStart it with: sudo systemctl start wardenclyffedisk",
            path.display()
        ).into());
    }
    ...
    if now - status.updated_at > 10 {
        return Err("Status file is stale. Is the wardenclyffedisk service running?".into());
    }
...
fn list_servers(path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("  WardenClyffeDisk Cluster");
    println!("  {}", "─".repeat(50));
    println!();
...
fn show_stats(path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    loop {
        ...
        match read_status(path) {
            Ok(status) => {
                println!();
                println!("  WardenClyffeDisk Cluster Stats");
                println!("  {}", "─".repeat(50));
                println!();
