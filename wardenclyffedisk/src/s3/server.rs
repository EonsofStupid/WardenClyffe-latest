//! S3-compatible HTTP server for WardenClyffeDisk
//!
//! Maps WardenClyffeDisk's file index and chunk store to S3 buckets and objects:
...
/// S3 server that runs alongside WardenClyffeDisk FUSE
pub struct S3Server {
    bind_addr: String,
    state: S3State,
}
...
async fn list_buckets(state: S3State) -> Response {
    ...
    xml.push_str("  <Owner>\n");
    xml.push_str("    <ID>wardenclyffedisk</ID>\n");
    xml.push_str("    <DisplayName>WardenClyffeDisk</DisplayName>\n");
    xml.push_str("  </Owner>\n");
