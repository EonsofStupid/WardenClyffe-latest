//! WardenClyffeDisk CLI
//!
//! Command-line interface for mounting and managing WardenClyffeDisk.
...
use wardenclyffedisk::{
    fuse::WardenClyffeDiskFS,
    storage::{ChunkRef, FileEntry, FileIndex},
    Config,
};

#[derive(Parser)]
#[command(name = "wardenclyffedisk")]
#[command(author = "WardenClyffe Software Systems Ltd")]
...
struct Cli {
    ...
    #[arg(short, long, default_value = "/etc/wardenclyffedisk/config.toml")]
    config: PathBuf,
...
#[derive(Subcommand)]
enum Commands {
    /// Mount the WardenClyffeDisk filesystem
    Mount {
        /// Mount point path
        #[arg(short, long)]
        mountpoint: PathBuf,
    ...
    /// Unmount the WardenClyffeDisk filesystem
    Unmount {
        /// Mount point path
        #[arg(short, long)]
        mountpoint: PathBuf,
    ...
    /// Initialize a new WardenClyffeDisk data directory
    Init {
        /// Data directory path
        #[arg(short, long, default_value = "/var/lib/wardenclyffedisk")]
        data_dir: PathBuf,
    },
}
...
fn main() {
    ...
    match cli.command {
        Commands::Mount { mountpoint } => {
            info!("Mounting WardenClyffeDisk at {:?}", mountpoint);
            info!("Node ID: {}, Role: {:?}", config.node.id, config.node.role);

            // Initialize cluster manager
            let mut cluster = wardenclyffedisk::ClusterManager::new(config.clone());
            ...
            // Create chunk store for replication (shared with WardenClyffeDiskFS)
            let chunk_store = std::sync::Arc::new(
                wardenclyffedisk::storage::ChunkStore::new(config.chunks_dir(), 4 * 1024 * 1024)
                    .expect("Failed to create chunk store"),
            );
            let chunk_store_for_handler = chunk_store.clone();

            // Build inode table from index (shared with WardenClyffeDiskFS)
            let (inode_table_data, max_inode) = {
                let index = file_index.read().unwrap();
                wardenclyffedisk::storage::InodeTable::from_index(&index)
            };
            ...
            let broadcast_queue: std::sync::Arc<
                std::sync::Mutex<Vec<(std::path::PathBuf, wardenclyffedisk::storage::FileEntry)>>,
            ...
            let is_client_role = config.node.role == wardenclyffedisk::config::NodeRole::Client;
            let cluster_for_handler = cluster.clone();

            // Create peer manager for network communication
            let peer_manager = std::sync::Arc::new(wardenclyffedisk::network::peer::PeerManager::new(
                config.node.id.clone(),
                config.node.bind.clone(),
                move |peer_id, msg| {
                    use wardenclyffedisk::network::protocol::*;
                    ...
                    match msg {
                        ...
                        Message::DeleteFile(del) => {
                            ...
                            if let Some(entry) = index.remove(&path) {
                                ...
                                let delete_marker = wardenclyffedisk::storage::FileEntry {
                        ...
                        Message::CreateSymlink(symlink_req) => {
                            ...
                            let entry = wardenclyffedisk::storage::FileEntry {
                        ...
                        Message::SyncRequest(sync_req) => {
                                ...
                                (&sync_req.node_id, &sync_req.bind_address)
                            {
                                ...
                                cluster_for_handler.update_peer(
                                    wardenclyffedisk::network::discovery::DiscoveredPeer {
                                        node_id: nid.clone(),
                                        address: actual_addr,
                                        role: if sync_req.is_client {
                                            wardenclyffedisk::network::discovery::DiscoveryRole::Client
                                        } else {
                                            wardenclyffedisk::network::discovery::DiscoveryRole::Server
                                        },
                                        is_leader: false,
            ...
            std::thread::spawn(move || {
                ...
                loop {
                    ...
                    for (path, mut entry) in pending {
                        ...
                        let chunk_refs: Vec<wardenclyffedisk::network::protocol::ChunkRefMsg> = entry
                            .chunks
                            .iter()
                            .map(|c| wardenclyffedisk::network::protocol::ChunkRefMsg {
                                hash: c.hash,
                                offset: c.offset,
                                size: c.size,
                            })
            ...
            let should_sync = config.node.role == wardenclyffedisk::config::NodeRole::Client
                || config.node.role == wardenclyffedisk::config::NodeRole::Follower;
            // For Auto role, we need to sync in the thread after election decides
            let is_auto_role = config.node.role == wardenclyffedisk::config::NodeRole::Auto;
            ...
            if should_sync || is_auto_role {
                ...
                let sync_is_client = config.node.role == wardenclyffedisk::config::NodeRole::Client;
                ...
                std::thread::spawn(move || {
                    ...
                    use wardenclyffedisk::network::protocol::*;
                    use wardenclyffedisk::storage::{ChunkRef, FileEntry};
                    ...
                    loop {
                        ...
                        if wait_count == 30 && !sync_peers.is_empty() {
                            ...
                            for peer_addr in &sync_peers {
                                ...
                                match wardenclyffedisk::network::peer::PeerConnection::connect(
                                    "probe".to_string(),
                                    peer_addr,
                                ) {
                                    Ok(conn) => {
            ...
            if should_sync || is_auto_role {
                ...
                let resync_is_client = config.node.role == wardenclyffedisk::config::NodeRole::Client;
            ...
            let fs = match WardenClyffeDiskFS::with_cluster(
                config.clone(),
                Some(cluster.clone()),
            ...
            // Mount options
            let options = vec![
                fuser::MountOption::FSName("wardenclyffedisk".to_string()),
                fuser::MountOption::AutoUnmount,
                fuser::MountOption::AllowOther,
            ];

            // Start status file writer thread for wardenclyffediskctl
            ...
            if config.s3.enabled {
                ...
                std::thread::spawn(move || {
                    ...
                    rt.block_on(async {
                        let server = wardenclyffedisk::s3::S3Server::new(
                            s3_bind,
                            s3_file_index,
                            s3_chunk_store,
                            s3_inode_table,
        ...
        Commands::Unmount { mountpoint } => {
            info!("Unmounting WardenClyffeDisk at {:?}", mountpoint);
        ...
        Commands::Status => {
            println!();
            println!("  WardenClyffeDisk Status");
            println!("  {}", "─".repeat(50));
            println!();
            ...
            println!();
            println!("Note: Run 'wardenclyffedisk stats' for live cluster statistics");
        }

        Commands::Stats => {
            println!();
            println!("  WardenClyffeDisk Live Stats");
            println!("  {}", "─".repeat(50));
            println!();
            ...
            while running.load(std::sync::atomic::Ordering::SeqCst) {
                ...
                println!("WardenClyffeDisk Cluster Stats - {} (Ctrl+C to exit)", now);
                println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                println!();
        ...
        Commands::ListServers { what } => {
            if what != "servers" {
                error!("Unknown list type '{}'. Use: wardenclyffedisk list servers", what);
                std::process::exit(1);
            }
            ...
            println!();
            println!("  WardenClyffeDisk Servers");
            println!("  {}", "─".repeat(50));
            println!();
            ...
            let my_role = if cluster.is_leader() {
                "LEADER"
            } else if cluster.state() == wardenclyffedisk::cluster::ClusterState::Client {
                "CLIENT"
            } else {
                "FOLLOWER"
            };
        ...
        Commands::Init { data_dir } => {
            info!("Initializing WardenClyffeDisk data directory at {:?}", data_dir);
