...
<div class="wiki-layout">
    ...
    <main class="wiki-content">
            ...
            <div class="content-section">
                <h2>Overview</h2>
                <img src="images/screenshots/ai-agent.png" alt="WardenClyffe AI Agent settings — Claude, Gemini, or local AI" class="screenshot" loading="lazy" style="border-radius:12px;border:1px solid var(--border-color);margin:1.5rem 0;">
                <p>WardenClyffe&rsquo;s AI Agent lets you ask questions about your infrastructure in natural language. Get intelligent answers about server health, resource usage, container status, and configuration &mdash; powered by leading AI models.</p>
                <p class="note"><strong>Looking for named, persistent-memory agents?</strong> See <a href="wardenclyffe-agents.php">WardenClyffeAgents</a> &mdash; a multi-agent surface that sits on top of this same AI configuration. The AI Agent page (this one) configures the <em>provider + key</em>; WardenClyffeAgents lets you create named personalities that each have their own system prompt, conversation history, tool allowlist, and optional Discord / Telegram / WhatsApp channel binding.</p>
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe can connect to <strong>self-hosted AI servers</strong> running on your own hardware &mdash; no cloud API keys required, no data leaves your network, and zero ongoing API costs.</p>
                ...
                <ul>
                    <li><strong>Ollama</strong> &mdash; the most popular local AI runtime. Run <code>ollama serve</code> and WardenClyffe connects automatically.</li>
                ...
                <p>When you point WardenClyffe at a local AI server, it <strong>automatically queries the available models</strong> and populates the model dropdown. No need to manually look up model names &mdash; just set the endpoint URL and WardenClyffe discovers what&rsquo;s installed.</p>

                <h3>Expert Knowledge Base</h3>
                <p>WardenClyffe ships with a <strong>built-in expert knowledge base</strong> that is injected into every AI conversation. This means even smaller local models can give deep, accurate answers about your infrastructure because they receive curated context about WardenClyffe&rsquo;s architecture, commands, and best practices alongside your question.</p>
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe&rsquo;s AI can run <strong>real queries against your databases</strong> using the same connection profiles the <a href="wardenclyffe-mysql.php">Database Manager</a> uses. Ask <em>"how many orders in the last 24 hours?"</em> and the AI writes the SQL, runs it, and feeds the numbers back into its answer &mdash; all routed through the cluster, all audit-logged, all enterprise-ACL-gated.</p>
                ...
                <h3>Audit trail</h3>
                <p>Every AI query is appended to <code>/var/log/wardenclyffe/sql-audit.log</code> with the agent ID, connection ID, full query text, outcome, row count, and elapsed time. You can review exactly what an agent asked the database at any point in the past.</p>
                <h3>Why this is safe</h3>
                <p>AI agents use the <strong>same cluster-aware pipeline</strong> as the UI &mdash; encrypted passwords at rest, cluster-secret authentication for inter-node proxying, enterprise per-user ACL on every profile, and the hardcoded API denylist that blocks agents from reaching <code>/api/sql-connections/*</code> through the generic <code>wardenclyffe_api</code> tool. The only path to SQL is the dedicated, gated <code>sql_query</code> tool.</p>
            </div>
            ...
            <div class="content-section">
                ...
                <ul>
                    ...
                    <li><strong>Full audit log</strong> &mdash; every proposed, approved, rejected, and executed action is logged to <code>/etc/wardenclyffe/ai-actions.log</code> with timestamps, usernames, and results</li>
                ...
                <p>When the AI health scanner detects an issue and proposes a fix, the <strong>alert email includes the proposed actions</strong> with their risk level and commands. You can then open the WardenClyffe dashboard to approve them.</p>
            </div>

            <div class="content-section">
                <h2>Health Monitoring</h2>
                <p>The AI Agent can run automated health checks across your infrastructure and send email alerts when it detects problems. This works alongside the <a href="wardenclyffe-issues.php">Issues Scanner</a> and <a href="wardenclyffe-alerting.php">threshold-based alerting</a> to provide an additional layer of intelligent monitoring.</p>
                <ul>
...
<div class="page-nav"><a href="app-store.php" class="prev">&larr; App Store</a><a href="wardenclyffe-wardenclyffenote.php" class="next">WardenClyffeNote Integration &rarr;</a></div>
