<?php
$page_title = 'Security, Users &amp; Authentication — WardenClyffe Docs';
$page_desc = 'WardenClyffe security features — user accounts, two-factor authentication (2FA), firewall management, Fail2ban, rate limiting, encryption, and best practices.';
$active = 'wardenclyffe-security.php';
...
<div class="wiki-layout">
    ...
    <main class="wiki-content" style="max-width:1100px;">
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe provides a complete security layer for your infrastructure &mdash; from user authentication
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe supports three authentication modes, configurable from <strong>Settings &rarr; Users &amp; Auth</strong>:</p>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin:16px 0;">
                    ...
                    <div style="background:var(--bg-tertiary);border:1px solid var(--border-color);border-radius:8px;padding:16px;">
                        <h3 style="font-size:0.92rem;margin-bottom:6px;">WardenClyffe Users Only</h3>
                        <p style="font-size:0.82rem;color:var(--text-secondary);">Disable Linux login entirely. Only WardenClyffe user accounts can log in. Supports optional two-factor authentication (2FA) per user.</p>
                    </div>
                    <div style="background:var(--bg-tertiary);border:1px solid var(--border-color);border-radius:8px;padding:16px;">
                        <h3 style="font-size:0.92rem;margin-bottom:6px;">Both</h3>
                        <p style="font-size:0.82rem;color:var(--text-secondary);">Allow either Linux system credentials or WardenClyffe user accounts. WardenClyffe users are checked first.</p>
                    </div>
                </div>

                <img src="images/screenshots/users-auth.png" alt="WardenClyffe Users &amp; Auth settings showing authentication mode selection and user management" class="screenshot" loading="lazy">
            </div>

            <!-- WardenClyffe User Accounts -->
            <div class="content-section">
                <h2>WardenClyffe User Accounts</h2>
                <p>Create dedicated WardenClyffe user accounts with their own passwords, independent of Linux system users. Each user has:</p>
                <ul>
                ...
                <p>User accounts are stored in <code>/etc/wardenclyffe/users.json</code> (configurable via <strong>Settings &rarr; File Locations</strong>).</p>

                <div class="warning-box">
                    <p><strong>Important:</strong> Before switching to <em>WardenClyffe Users Only</em> mode, create at least one WardenClyffe user account. Otherwise you will be locked out of the dashboard.</p>
                </div>
            </div>
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe supports TOTP-based two-factor authentication, compatible with:</p>
                <ul>
                ...
                <div class="info-box">
                    <p><strong>Clock skew tolerance:</strong> WardenClyffe accepts codes from the current time step and &plusmn;1 step (30 seconds each way), so minor clock differences between your server and phone won&rsquo;t cause issues.</p>
                </div>
            </div>
            ...
            <div class="content-section">
                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>Shared secret sent via <code>X-WardenClyffe-Secret</code> header. Generate a custom secret in Settings &rarr; Security.</td>
                        </tr>
                        <tr>
                            <td><strong>Join tokens</strong></td>
                            <td>Adding new nodes</td>
                            <td>Per-node tokens for cluster join operations. View with <code>wardenclyffe --show-token</code>.</td>
                        </tr>
                        <tr>
                ...
                <img src="images/screenshots/settings-security.png" alt="WardenClyffe cluster security settings showing custom cluster secret management" class="screenshot" loading="lazy">
            </div>
            ...
            <div class="content-section">
                ...
                <p>WardenClyffe provides a web interface for managing Linux firewall tools on each node.</p>

                <img src="images/screenshots/security.png" alt="WardenClyffe security view showing Fail2ban, UFW, and iptables management" class="screenshot" loading="lazy">
            ...
            <div class="content-section">
                <h2>Encryption</h2>
                <ul>
                    <li><strong>WardenClyffeNet</strong> &mdash; All inter-node traffic is encrypted with X25519 key exchange and ChaCha20-Poly1305 symmetric encryption</li>
                    ...
                    <li><strong>Password storage</strong> &mdash; WardenClyffe user passwords are hashed with SHA-512 crypt (never stored in plaintext)</li>
                    <li><strong>TOTP secrets</strong> &mdash; Stored server-side in <code>/etc/wardenclyffe/users.json</code> (root-only readable)</li>
                </ul>
            </div>
            ...
            <div class="content-section">
                ...
                <ul>
                    ...
                    <li>WardenClyffe itself can be updated via <strong>WardenClyffeFlow</strong> automation or the Issues page</li>
                </ul>
            </div>

            <!-- AI & WardenClyffeAgents -->
            <div class="content-section">
                <h2>AI &amp; WardenClyffeAgents</h2>
                <p>Enabling AI in WardenClyffe &mdash; Settings &rarr; AI Agent, or any named WardenClyffeAgent &mdash; changes your threat model. The LLM sees untrusted text (chat messages, log tails, alert payloads) and can invoke tools. Here's what to do, not just what to worry about.</p>
                ...
                <ul>
                    ...
                    <li>Fill in <strong>Allowed paths</strong> before granting <code>write_file</code> or <code>schedule_script</code>. <code>/home/wardenclyffegrid1/assetcache</code> is a scope; <code>/</code> is an incident waiting to happen.</li>
                    <li>If granting the Enterprise <code>wardenclyffe_api</code> tool, fill in <strong>Allowed API paths</strong>. A good starting set: <code>/api/nodes, /api/containers, /api/metrics, /api/alerts</code>. Never leave it empty.</li>
                ...
                <ul>
                    <li><strong>AI provider API keys</strong> (Anthropic, Gemini, OpenRouter) live in Settings &rarr; AI Agent. Rotate them when an operator leaves, after any leak suspicion, or at least every 90 days. WardenClyffe stores them write-only; the UI never echoes them back.</li>
                    ...
                    <li><strong>Cluster secret</strong> &mdash; the universal <code>wardenclyffe_api</code> tool authenticates loopback calls with it. Rotate on incident (Settings &rarr; Security &rarr; Regenerate), then re-pair your nodes.</li>
                </ul>

                <h3>Keep prompts and memory private</h3>
                <ul>
                    <li>Memory and audit logs live at <code>0o600</code> in <code>/etc/wardenclyffe/agents/&lt;id&gt;/</code> with directories at <code>0o700</code>. If you have untrusted local users on a WardenClyffe host, don't put them in the <code>wardenclyffe</code> group.</li>
                ...
                <ul>
                    <li>WardenClyffe hardcodes denies for <code>rm -rf /</code>, disk wipes, firewall flushes, curl-pipe-bash, and access to <code>/etc/shadow</code>, <code>/etc/passwd</code>, <code>/boot</code>, and WardenClyffe's own config. These apply <em>above</em> access level &mdash; even a Trusted agent can't bypass them, and an operator can't approve around them.</li>
            ...
            <div class="content-section">
                ...
                <ul>
                    <li>Switch to <strong>WardenClyffe Users Only</strong> mode and enable 2FA for all users</li>
                    ...
                    <li>Use <strong>WardenClyffeNet</strong> to keep management traffic off the public internet</li>
                    <li>Keep WardenClyffe updated to the latest version</li>
                    <li>Disable direct login on secondary nodes &mdash; manage them through the primary dashboard</li>
                </ul>
            </div>

<div class="page-nav"><a href="wardenclyffe-mysql.php" class="prev">&larr; Databases</a><a href="wardenclyffe-certificates.php" class="next">Certificates &rarr;</a></div>
