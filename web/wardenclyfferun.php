<?php
$page_title = 'WardenClyffeRun Orchestration — WardenClyffe Docs';
$page_desc = 'WardenClyffeRun — native container orchestration for WardenClyffe. Schedule, scale, load-balance, and failover Docker & LXC services across your cluster with zero external dependencies.';
$page_keywords = 'WardenClyffeRun, container orchestration, Docker, LXC, scaling, failover, HA, load balancing, WardenClyffe, cluster scheduling, service mesh';
$active = 'wardenclyfferun.php';
...
<div class="wiki-layout">
    ...
    <main class="wiki-content">
            ...
            <div class="content-section">
                <h1>Overview</h1>
                <p>WardenClyffeRun is WardenClyffe&rsquo;s native container orchestration engine. It lets you define <strong>services</strong> that are automatically scheduled, scaled, load-balanced, and managed across your cluster nodes &mdash; similar to Docker Swarm or Kubernetes Deployments, but built directly into WardenClyffe with support for both Docker <em>and</em> LXC containers.</p>
                <img src="screenshots/wardenclyfferun.png" alt="WardenClyffeRun container orchestration dashboard" class="screenshot" loading="lazy">
                <p>WardenClyffeRun requires <strong>zero external dependencies</strong> &mdash; no etcd, no kubelet, no CNI plugins. Everything runs inside the WardenClyffe binary. Container networking is handled automatically by <a href="wardenclyffenet.php">WardenClyffeNet</a>.</p>
                ...
                <ul>
                    ...
                    <li><strong>Built-in load balancing</strong> &mdash; Each service gets a WardenClyffeNet VIP with round-robin or IP-hash (sticky session) routing</li>
                    ...
                    <li><strong>Cross-node cloning</strong> &mdash; Clone and migrate LXC containers across nodes with automatic WardenClyffeNet IP assignment</li>
                    ...
                    <li><strong>Restart policies</strong> &mdash; Always, OnFailure, or Never &mdash; WardenClyffeRun automatically restarts stopped containers</li>
                    <li><strong>Adopt existing containers</strong> &mdash; Bring existing Docker or LXC containers under WardenClyffeRun management without recreating them</li>
                    <li><strong>Port forwarding</strong> &mdash; Forward external ports on your server to a service&rsquo;s WardenClyffeNet VIP</li>
                    <li><strong>App Store integration</strong> &mdash; Deploy applications from the WardenClyffe App Store directly into WardenClyffeRun</li>
                </ul>
            </div>

            <div class="content-section">
                <h2>How WardenClyffeRun Works</h2>
                <p>WardenClyffeRun uses a <strong>leader-based reconciliation loop</strong> that runs every 15 seconds. The cluster leader (lowest alphabetical node ID among online nodes) is responsible for all orchestration decisions:</p>
                <ol>
                    ...
                    <li><strong>Scale up</strong> &mdash; If there are fewer active instances than the desired replica count, WardenClyffeRun clones or creates new containers on eligible nodes</li>
                    ...
                    <li><strong>Restart</strong> &mdash; If a container has exited or stopped and the restart policy is &ldquo;Always&rdquo;, WardenClyffeRun restarts it automatically</li>
            ...
            <div class="content-section">
                ...
                <p>Navigate to the <strong>WardenClyffeRun</strong> page in the datacenter sidebar and click <strong>Deploy Service</strong>.</p>
                ...
                <p>WardenClyffeRun will pull the image on the target node, create the container, start it, assign a WardenClyffeNet IP, and register it as a managed instance. If you requested multiple replicas, containers are spread across eligible cluster nodes.</p>
                ...
                <p>WardenClyffeRun creates the first LXC container from the distribution template, then <strong>clones</strong> it across nodes for additional replicas. Each clone gets a unique bridge IP and WardenClyffeNet IP automatically.</p>

                <div class="warning-box" style="margin-top:1rem;">
                    <p><strong>LXC cross-node cloning:</strong> When WardenClyffeRun needs to place an LXC replica on a different node, it performs a full rootfs clone-and-migrate over the network. This can take 30&ndash;120 seconds depending on the container size and network speed. The instance shows as &ldquo;pending&rdquo; in the dashboard until the migration completes.</p>
                </div>
            </div>
            ...
            <div class="content-section">
                ...
                <h3>Dashboard</h3>
                <p>The WardenClyffeRun dashboard shows all services with real-time status:</p>
                <ul>
                    ...
                    <li><strong>Instance list</strong> &mdash; Click a service row to expand and see every instance with container name, node, WardenClyffeNet IP, and status. Standby instances are shown with a purple STANDBY badge at reduced opacity</li>
                    <li><strong>Service VIP</strong> &mdash; The WardenClyffeNet virtual IP assigned to this service for load-balanced access</li>
                </ul>
                ...
                <ul>
                    ...
                    <li>WardenClyffeRun respects the min/max bounds &mdash; you cannot scale below the minimum or above the maximum</li>
                ...
                <ul>
                    ...
                    <li><strong>Min replicas</strong> &mdash; Scale-down floor (WardenClyffeRun will never remove instances below this count)</li>
                ...
                <ol>
                    ...
                    <li>WardenClyffeRun pulls the new image on each node and replaces instances one at a time</li>
                </ol>
                <p>The load balancer continues routing traffic to healthy instances while the update rolls out.</p>

                <h3>Deleting a Service</h3>
                <p>Deleting a service stops and destroys all cloned instances (containers with &ldquo;wardenclyfferun&rdquo; in their name) and all standby instances. The original template container is preserved so you can re-deploy or use it standalone.</p>
            </div>

            <div class="content-section">
                <h2>Load Balancing</h2>
                <p>Every WardenClyffeRun service is automatically assigned a <strong>WardenClyffeNet Virtual IP (VIP)</strong> from the <code>10.10.10.x</code> range. The VIP acts as a stable endpoint that load-balances traffic across all healthy instances.</p>

                <h3>How It Works</h3>
                <ol>
                    <li>When a service is created, WardenClyffe allocates a unique WardenClyffeNet IP as the service VIP</li>
                    ...
                    <li>The reconciliation loop updates the backend list every 15 seconds &mdash; only <strong>running, non-standby</strong> instances with a WardenClyffeNet IP are included</li>
                    <li>The VIP is announced on WardenClyffeNet via gratuitous ARP so other nodes can route to it</li>
                </ol>
                ...
                <p>By default, the service VIP is only accessible from your WardenClyffeNet mesh. To expose a service externally, use the <strong>Port Forward</strong> button on a service to create iptables rules that forward a port on your server&rsquo;s public IP to the service VIP.</p>
  ...
  curl http://10.10.10.42:80
    ...
    &rarr; round-robin repeats...</code></pre>
            ...
            <div class="content-section" id="failover">
                ...
                <p>WardenClyffeRun&rsquo;s failover system provides <strong>automatic high availability</strong> for your services. When enabled, WardenClyffeRun pre-creates stopped &ldquo;standby&rdquo; containers on other cluster nodes. If a node hosting active containers goes offline, standby containers are instantly promoted &mdash; no re-imaging, no IP reassignment, no manual intervention.</p>
                ...
                <ol>
                    ...
                    <li>WardenClyffeRun automatically <strong>creates standby containers</strong> on cluster nodes that don&rsquo;t already have an instance of this service. For Docker, this means pulling the image and creating the container (stopped). For LXC, this means cloning the template (stopped)</li>
                    <li>Each standby gets a <strong>pre-allocated WardenClyffeNet IP</strong> so there&rsquo;s no IP assignment delay during promotion</li>
                ...
                <div class="code-block" style="margin:0.5rem 0 1rem 0;">
                    ...
                    <pre><code>Before:
  node-a (online)   &rarr;  myapp-wardenclyfferun-abc123  [RUNNING]
...
After (within 15 seconds):
  node-a (offline)  &rarr;  myapp-wardenclyfferun-abc123  [removed from tracking]
  node-b (online)   &rarr;  myapp-standby-def456  [RUNNING, promoted]
  node-c (online)   &rarr;  myapp-standby-ghi789  [STANDBY, stopped]

Failover event logged:
  "Standby 'myapp-standby-def456' promoted &mdash; replacing failed 'myapp-wardenclyfferun-abc123'"

Next cycle: WardenClyffeRun creates a new standby on node-c (or another eligible node)
to restore full HA coverage.</code></pre>
                ...
                <ul>
                    ...
                    <li><strong>Failover events</strong> are persisted to disk and shown in the WardenClyffeRun dashboard as a log table with timestamps, service names, source/destination nodes, and details</li>
            ...
            <div class="content-section">
                ...
                <p>You can bring an existing Docker or LXC container under WardenClyffeRun management without recreating it:</p>
                <ol>
                    <li>Navigate to the container in the WardenClyffe dashboard</li>
                    <li>Click <strong>Adopt into WardenClyffeRun</strong></li>
                    <li>Enter a service name &mdash; the container becomes the first instance of a new WardenClyffeRun service</li>
                    <li>WardenClyffeRun registers the container, assigns a service VIP, and begins managing it</li>
                </ol>
                <p>Once adopted, you can scale the service to create additional replicas cloned from the original container. The original container is never destroyed &mdash; even if you later delete the WardenClyffeRun service, the original template is preserved.</p>
            </div>

            <div class="content-section">
                <h2>Placement &amp; Scheduling</h2>
                <p>WardenClyffeRun uses a <strong>spread scheduler</strong> that distributes instances across nodes to maximise availability. The scheduler scores each eligible node based on:</p>
                <ul>
                ...
                <p>A node is eligible to receive a WardenClyffeRun instance if:</p>
                <ul>
            ...
            <div class="content-section">
                <h2>Networking</h2>
                <p>WardenClyffeRun relies on <a href="wardenclyffenet.php">WardenClyffeNet</a> for all container networking:</p>
                <ul>
                    <li>Every instance gets a <strong>WardenClyffeNet IP</strong> on the <code>10.10.10.x</code> subnet, making it reachable from any node in your cluster</li>
                    <li>The service <strong>VIP</strong> is also a WardenClyffeNet IP that load-balances across healthy instances</li>
                    <li>Cross-node cloning automatically assigns unique WardenClyffeNet IPs to avoid conflicts</li>
                    <li>WardenClyffeNet routes are announced via gratuitous ARP and pushed into the WardenClyffeNet routing table so all peers can reach the service</li>
                </ul>
                <p>If WardenClyffeNet is not configured, containers will only have their local bridge IP and cross-node access will not work. Setting up WardenClyffeNet before using WardenClyffeRun is strongly recommended.</p>
            </div>

            <div class="content-section">
                <h2>WardenClyffeRun vs. WardenClyffeKube</h2>
                <p>WardenClyffe includes two orchestration systems for different use cases:</p>
                <div class="table-wrapper">
                    <table class="data-table">
                        <thead>
                            <tr>
                                <th>Feature</th>
                                <th>WardenClyffeRun</th>
                                <th>WardenClyffeKube</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr><td>Technology</td><td>WardenClyffe-native orchestration</td><td>Kubernetes (k3s, MicroK8s, kubeadm, k0s, RKE2)</td></tr>
                            <tr><td>Container Runtime</td><td>Docker &amp; LXC</td><td>containerd</td></tr>
                            <tr><td>Setup</td><td>No setup &mdash; built into WardenClyffe</td><td>Provision via WardenClyffeKube wizard</td></tr>
                            ...
                            <tr><td>Load Balancing</td><td>WardenClyffeNet VIP (iptables DNAT)</td><td>WardenClyffeNet private IPs + Kubernetes Service routing</td></tr>
                            <tr><td>Networking</td><td>WardenClyffeNet mesh</td><td>Kubernetes CNI + WardenClyffeNet</td></tr>
                            <tr><td>Ecosystem</td><td>WardenClyffe-only</td><td>Full Kubernetes ecosystem (Helm, operators, CRDs)</td></tr>
                ...
                <p>You can run both WardenClyffeRun and WardenClyffeKube on the same cluster. Use WardenClyffeRun for quick deployments, LXC-based services, and simple scaling. Use WardenClyffeKube when you need the full Kubernetes ecosystem.</p>
            </div>

            <div class="content-section">
                <h2>Configuration &amp; Persistence</h2>
                <p>WardenClyffeRun stores all state as JSON files in <code>/etc/wardenclyffe/wardenclyfferun/</code>:</p>
                <ul>
                ...
                <p>There is no database &mdash; everything is file-based. You can back up the <code>/etc/wardenclyffe/wardenclyfferun/</code> directory to preserve your WardenClyffeRun configuration.</p>
            </div>
            ...
            <div class="content-section">
                ...
                <ul>
                    ...
                    <li>Check the WardenClyffe logs (<code>journalctl -u wardenclyffe</code>) for &ldquo;WardenClyffeRun: no eligible nodes&rdquo; messages</li>
                ...
                <ul>
                    ...
                    <li>Cross-node clones require SSH access between nodes. WardenClyffe handles this automatically if nodes are in the same cluster</li>
                ...
                <ul>
                    <li>Verify WardenClyffeNet is running on the leader node (<code>ip addr show wardenclyffenet0</code>)</li>
                    <li>Check that instances have WardenClyffeNet IPs assigned (visible in the instance list)</li>
                ...
                <ul>
                    ...
                    <li>Check the failover events log at the bottom of the WardenClyffeRun page</li>
                </ul>
            </div>

<div class="page-nav"><a href="wardenclyffe-backups.php" class="prev">&larr; Backup &amp; Restore</a><a href="wardenclyffekube.php" class="next">WardenClyffeKube Kubernetes &rarr;</a></div>
