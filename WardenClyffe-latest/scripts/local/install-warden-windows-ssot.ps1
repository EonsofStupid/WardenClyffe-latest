param(
  ...
  # SSH topology - every host below routes through $ProxyHost as the public jump.
  [string]$ProxyHost = "ssh.clyffy.ai",
  [string]$ProxyUser = "root",
  [string]$WindowsSshPath = "C:\Windows\System32\OpenSSH\ssh.exe",

  # Devstation (VM 116) - single host entry covers all four aliases; do not
  # duplicate `devstation` or `warden-devstation` elsewhere in this block.
  [string]$DevstationIp = "10.0.0.116",
  [string]$DevstationUser = "wardenop",

  # Devstation browser-IDE tunnel - separate Host entry because of LocalForward,
  # but it MUST NOT reuse the `devstation` / `warden-devstation` aliases.
  [int]$DevstationCodeLocalPort = 18080,
  [int]$DevstationCodeRemotePort = 8080,

  # Warden Operator Capsule (LXC 114).
  [string]$CapsuleIp = "10.0.0.114",
  [string]$CapsuleUser = "wardenop",

  # Optional absorption: pull the `# BEGIN/END WardenClyffe server1` block
  # under the SSOT markers so there's truly one managed authority.
  [switch]$IncludeServer1,
  [string]$Server1Host = "server1",
  [string]$Server1User = "root",

  [switch]$NoSshConfig,
  [switch]$OnlySshConfig,
  [switch]$NoOpen,
  [switch]$Force
)
...
Host devstation warden-devstation devstation.clyffy.ai warden-devstation.clyffy.ai
  HostName $DevstationIp
  User $DevstationUser
  IdentityFile $SshIdentityFile
  IdentitiesOnly yes
  ProxyCommand "$WindowsSshPath" -W %h:%p $ProxyHost
  StrictHostKeyChecking accept-new

Host warden-capsule capsule.clyffy.ai operator.clyffy.ai warden-capsule.clyffy.ai
  HostName $CapsuleIp
  User $CapsuleUser
  IdentityFile $SshIdentityFile
  IdentitiesOnly yes
...
Host warden-devstation-code code.devstation.clyffy.ai code.warden-devstation.clyffy.ai
  ...
  ProxyCommand "$WindowsSshPath" -W %h:%p $ProxyHost
  StrictHostKeyChecking accept-new
  LocalForward 127.0.0.1:$DevstationCodeLocalPort 127.0.0.1:$DevstationCodeRemotePort
  ExitOnForwardFailure yes
  ServerAliveInterval 5
  ServerAliveCountMax 3$server1Segment
$ManagedBlockEnd
...
if ($OnlySshConfig) {
  Write-Step "OnlySshConfig set - skipping SMB mount and credential broker"
  return [pscustomobject]@{
    ssh_config = "ok"
    storage_mapping = "skipped"
    devstation_ssh = "not_tested"
  }
}
