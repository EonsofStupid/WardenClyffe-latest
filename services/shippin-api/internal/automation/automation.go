...
// (shippin_core.action_requests) and the provision workflow entrypoint.
package automation
...
func (s *Service) Provision(ctx context.Context, in ProvisionInput) (*ProvisionResult, error) {
	...
	err := s.db.QueryRow(ctx,
		`INSERT INTO shippin_core.action_requests (tenant_id, kind, status, payload)
	...
	spec := map[string]any{"repo": in.Repo, "region": in.Region, "requested_via": "shippin-api"}
	...
	_, _ = s.db.Exec(ctx,
		`UPDATE shippin_core.action_requests SET result = jsonb_build_object('resource_id',$2::text) WHERE id=$1`,
		actionID, res.ID)
