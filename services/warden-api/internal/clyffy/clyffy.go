...
// reads Warden's CAPTURED model of the foundation (warden_infra.platforms /
...
func (st *Store) queryServices(ctx context.Context, where, arg string) ([]Service, error) {
	sql := `SELECT ` + svcCols + ` FROM warden_infra.services s
	        LEFT JOIN warden_infra.platforms p ON p.id = s.platform_id ` + where +
...
func (st *Store) ListPlatforms(ctx context.Context) ([]Platform, error) {
	rows, err := st.db.Query(ctx,
		`SELECT stable_id, kind::text, api_endpoint, network_name, managed, credential_ref, state, note
		 FROM warden_infra.platforms ORDER BY kind`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
