...
// warden_core.schema_migrations ledger (created by 0001 and self-registered
...
func Applied(ctx context.Context, db *pgxpool.Pool) (map[string]bool, error) {
	...
	err := db.QueryRow(ctx, `SELECT EXISTS (
	    ...
	    WHERE n.nspname='warden_core' AND c.relname='schema_migrations')`).Scan(&exists)
	if err != nil {
		return nil, err
	}
	...
	rows, err := db.Query(ctx, `SELECT version FROM warden_core.schema_migrations`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func Up(ctx context.Context, db *pgxpool.Pool, dir string) ([]string, error) {
	...
	for _, m := range migrations {
		...
		if _, err := db.Exec(ctx,
			`INSERT INTO warden_core.schema_migrations(version) VALUES ($1)
