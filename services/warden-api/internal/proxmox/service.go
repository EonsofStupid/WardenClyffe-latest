...
func (s *Service) runGuestAction(ctx context.Context, kind, node, guestKind string, vmid int, act guestActor) (*ActionResult, error) {
	...
	err := s.db.QueryRow(ctx,
		`INSERT INTO warden_core.action_requests (kind, status, payload)
	...
	_, _ = s.db.Exec(ctx,
		`UPDATE warden_core.action_requests SET status='approved', updated_at=now() WHERE id=$1`, actionID)
	_, _ = s.db.Exec(ctx,
		`UPDATE warden_core.action_requests SET status='executing', updated_at=now() WHERE id=$1`, actionID)
	...
	if err != nil {
		_, _ = s.db.Exec(ctx,
			`UPDATE warden_core.action_requests SET status='failed', error=$2, updated_at=now() WHERE id=$1`,
			actionID, err.Error())
		_ = s.audit.Append(ctx, audit.Event{
	...
	_, _ = s.db.Exec(ctx,
		`UPDATE warden_core.action_requests
...
func (s *Service) GetAction(ctx context.Context, id string) (map[string]any, error) {
	...
	err := s.db.QueryRow(ctx,
		`SELECT kind, status::text, payload, result, error, created_at, updated_at
		 FROM warden_core.action_requests WHERE id=$1`, id).
