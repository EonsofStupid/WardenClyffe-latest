// Package edge owns the public-IP inventory in warden_infra: the addresses
...
// The transition log (warden_infra.ip_migrations) references these addresses.
package edge
...
func (s *Store) ListIPs(ctx context.Context) ([]PublicIP, error) {
	rows, err := s.db.Query(ctx,
		`SELECT `+cols+` FROM warden_infra.public_ips ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CreateIP(ctx context.Context, in CreateIPInput) (*PublicIP, error) {
	...
	rows, err := s.db.Query(ctx,
		`INSERT INTO warden_infra.public_ips (address, role, status, provider, label, note)
		 VALUES ($1::inet, $2::warden_infra.public_ip_role, $3::warden_infra.public_ip_status, $4, $5, $6)
		 RETURNING `+cols,
...
func (s *Store) UpdateIP(ctx context.Context, id string, in UpdateIPInput) (*PublicIP, error) {
	rows, err := s.db.Query(ctx,
		`UPDATE warden_infra.public_ips SET
		   role     = COALESCE($2::warden_infra.public_ip_role, role),
		   status   = COALESCE($3::warden_infra.public_ip_status, status),
