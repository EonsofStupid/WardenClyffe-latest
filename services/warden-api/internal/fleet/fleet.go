...
// warden_infra. It is read-authority for the console's workspace views.
package fleet
...
func (s *Store) List(ctx context.Context) ([]Resource, error) {
	rows, err := s.db.Query(ctx,
		`SELECT `+selectCols+` FROM warden_infra.resources ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) Get(ctx context.Context, id string) (*Resource, error) {
	rows, err := s.db.Query(ctx,
		`SELECT `+selectCols+` FROM warden_infra.resources WHERE id=$1`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CreateRequested(ctx context.Context, stableID, tier string, tenantID *string, spec map[string]any) (*Resource, error) {
	...
	rows, err := s.db.Query(ctx,
		`INSERT INTO warden_infra.resources (stable_id, tenant_id, kind, state, tier, spec)
